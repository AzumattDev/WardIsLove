...
public class GradientPicker : MonoBehaviour
{
    ...
    public static bool Create(Gradient original, string message, GradientEvent onGradientChanged,
        GradientEvent onGradientSelected)
    {
        if (instance is null)
        {
    ...
    private void CreateAlphaKey(GradientAlphaKey k)
    {
        if (alphaKeys.Count < 8)
        {
            ...
            s.gameObject.SetActive(true);
            s.value = k.time;
            s.transform.GetChild(0).GetChild(0).GetChild(0).GetComponent<Image>().color =
                new Color(k.alpha, k.alpha, k.alpha, 1f);
    ...
    public void SetAlpha(float value)
    {
        if (interact)
        {
            ...
            alphaKeyObjects[selectedAlphaKey].transform.GetChild(0).GetChild(0).GetChild(0).GetComponent<Image>()
    ...
    private void ChangeSelectedColorKey(int value)
    {
        ...
        {
            colorKeyObjects[selectedColorKey].transform.GetChild(0).GetChild(0).GetComponent<Image>().color =
                Color.gray;
        }

        if (alphaKeyObjects.Count() > 0)
        {
            alphaKeyObjects[selectedAlphaKey].transform.GetChild(0).GetChild(0).GetComponent<Image>().color =
                Color.gray;
        }
    ...
    public void CheckDeleteKey(Slider s)
    {
        if (Input.GetMouseButtonDown(1))
        {
            switch (s.name)
            {
                case "ColorKey" when colorKeys.Count > 2:
                {
                    if (!ColorPicker.done)
                    {
                        ColorPicker.Done();
                ...
                case "AlphaKey" when alphaKeys.Count > 2:
                {
    ...
    public void Select()
    {
        ...
        else
        {
            ...
            alphaComponent.GetChild(4).GetComponent<InputField>().text =
                Mathf.RoundToInt(alphaKeys[selectedAlphaKey].alpha * 255f).ToString();
        }
    }
