...
namespace WardIsLove.Extensions
{
    public class DragControl : MonoBehaviour
    {
        ...
        public void Drag()
        {
            ...
            Vector2 Transform = new(window.rect.width * transform.root.lossyScale.x,
            ...
            else if (OffsetMax.x > Screen.width) newPos.x = Screen.width - (1 - window.pivot.x) * Transform.x;
            if (OffsetMin.y < 0)
                newPos.y = window.pivot.y * Transform.y;
            else if (OffsetMax.y > Screen.height) newPos.y = Screen.height - (1 - window.pivot.y) * Transform.y;
